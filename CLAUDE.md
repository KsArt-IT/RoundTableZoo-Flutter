...
- **Чем анимировать персонажей.** `lottie: 3.5.1` подключён, ветка `Lottie.asset` в
  ...
  `CharacterReaction.intensity`). Нарисованы кот (лежит на боку) и пёс (сидит); крокодил и
  бегемот остаются на эмодзи-аватаре, пока им не добавят `FaceShape` и `AnimalShape`.
