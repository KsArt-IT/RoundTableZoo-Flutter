...
- **Чем анимировать персонажей.** `lottie: 3.5.1` подключён, ветка `Lottie.asset` в
  ...
  `CharacterReaction.intensity`). Нарисован весь ростер: кот (лежит на боку), пёс (сидит),
  крокодил (лежит плашмя) и бегемот (стоит на четырёх). У крокодила рот — не эллипс, а нижняя
  челюсть на шарнире, и её углы **отрицательные**: морда смотрит влево, положительный поворот
  поднимал бы челюсть в череп. У бегемота дальняя пара ног помечена `behind` и рисуется под
  телом — иначе четыре ноги встают рядком без глубины.
  Все фигуры нарисованы **смотрящими влево**; место на левой половине стола отражается по
  горизонтали (`RoundTableLayout.seatFacesRight` → `CharacterAvatar.mirrored`), иначе персонаж
  сидит спиной к столу. Формула угла места живёт в одном месте — `RoundTableLayout.seatAngle`.
  ...
  **Не выпиливать Lottie и не добавлять Rive без явного решения.** Разбор —
  ...
  (вопрос 8).
  **На Столе виден только один бабл** — последнего тронутого персонажа, при открытии экрана
  персонажа с самой поздней репликой (FR-017c). Остальные остаются в состоянии «уже отвечал»,
  их реплики — в Дневнике.
