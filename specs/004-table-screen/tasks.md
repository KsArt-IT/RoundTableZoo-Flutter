...
- [x] T019 Подменять `ShareService` на мок в `app/test/support/test_app_root.dart`, чтобы widget-тесты не уходили в платформенный канал (урок про `MissingPluginException`) — **выполнено в Phase 7/US5** вместе с T070 (`_useFakeShareService` в `test_app_root.dart`, тот же приём, что и `NotificationScheduler`); чекбокс не был отмечен при закрытии T070 — исправлено здесь без содержательных изменений
...
- [x] T069 [P] [US5] Написать проверку в `app/test/widget/table_page_test.dart`: долгое нажатие по баблу вызывает `ShareService` с текстом реплики и именем персонажа (мок из `test/support/mocks.dart`), короткий тап — не вызывает — добавлен `MockShareService` в `test/support/mocks.dart`, `verifyNever`/`verify(...).captured` подтверждают оба условия; тест увеличивает вьюпорт (`tester.view.physicalSize`), иначе бабл верхнего/правого места (открывается вверх — `round_table_layout.dart`) частично уходит за пределы дефолтного 800×600 и `longPress` промахивается

### Implementation for User Story 5

- [x] T070 [P] [US5] Создать `app/lib/core/sharing/share_service.dart` — интерфейс `ShareService` и реализация поверх `share_plus` (API — по результату T002), регистрация в `injection_module.dart` (research.md R3) — `SharePlusShareService.shareText()` вызывает `SharePlus.instance.share(ShareParams(text: text))` (подтверждено на `share_plus-13.3.0` в T002); зарегистрирован `@lazySingleton`; попутно закрыт отложенный T019 — `ShareService` подменяется на `MockShareService` в `test/support/test_app_root.dart` (`_useFakeShareService`), тем же приёмом, что и `NotificationScheduler`
- [x] T071 [US5] Развести жесты в `app/lib/presentation/table/widgets/speaking_bubble.dart`: короткий тап раскрывает реплику целиком, долгое нажатие делится (FR-017a, FR-030) — `SpeakingBubble` принимает `characterName`/`shareService` от вызывающей стороны (`table_page.dart` прокидывает через `_TableContent`/`_RoundTable`, как и остальные пер-персонажные данные), текст «`имя: реплика`»
- [x] T072 [US5] Добавить именованное действие «поделиться» в семантику бабла в `app/lib/presentation/table/widgets/speaking_bubble.dart`, чтобы жест не был единственным способом (FR-030) — `Semantics.customSemanticsActions` с `CustomSemanticsAction(label: l10n.tableShareAction)` (класс живёт в `package:flutter/semantics.dart`, не реэкспортируется `material.dart`/`widgets.dart` — отдельный импорт)
- [x] T073 [US5] Прогнать `flutter analyze`, `flutter test`, ручной прогон строки «поделиться» из `quickstart.md` §2 — 0 ошибок analyze, 227 тестов зелёных (226 + 1 новый T069); ручной прогон на симуляторе не завершён в этой сессии — сборка iOS зависла на разрешении Swift Package (см. предупреждение об SPM-миграции при `flutter run`, вероятно нет сетевого доступа в песочнице) и была прервана; поведение долгого нажатия/короткого тапа и содержимое передаваемого текста подтверждены widget-тестом T069 через мок `ShareService`, реального системного диалога не видели
...
- [x] T074 [P] Написать `app/test/widget/table_accessibility_test.dart`: `meetsGuideline(labeledTapTargetGuideline)`, `meetsGuideline(androidTapTargetGuideline)`, наличие метки состояния у каждого персонажа (SC-008); флаги семантики сравнивать с `Tristate.isTrue`, а не с `bool` (`lessons-learned.md`) — файл создан, два теста: гайдлайны на `/table` при первом открытии и `findsNWidgets(4)` по суффиксу `tableCharacterStateIdle`
- [x] T075 [P] Реализовать явный порядок обхода фокусом в `app/lib/presentation/table/table_page.dart`: шкала → поле текста → персонажи по кругу → бабл активного (FR-035) — новый `TableTraversalOrder` (0=шкала, 1=текст, `avatar(i)=2+i` по посадочному порядку, `bubble(i)=avatar(i)+0.5` для неактивных, `activeBubble=1000` для активного) применён одновременно как `Semantics.sortKey` (программы чтения) и `FocusTraversalOrder` внутри `FocusTraversalGroup(policy: OrderedTraversalPolicy())` (клавиатура); `CharacterAvatar` переведён с `GestureDetector` на `InkResponse` (тот же приём, что уже был в `MoodScaleRow._MoodOption`), иначе место не участвовало в Tab-обходе вообще — без фокусируемого виджета `FocusTraversalOrder` не на чем применять; `package:flutter/semantics.dart` понадобился отдельным импортом — `OrdinalSortKey` не реэкспортируется `material.dart` (см. `lessons-learned.md`)
- [x] T076 [P] Реализовать объявление новой реплики программе чтения с экрана в `app/lib/presentation/table/widgets/speaking_bubble.dart` (FR-036) — уже реализовано в US2 (T047): `Semantics(liveRegion: true, ...)` переобъявляется при каждой смене лейбла (новый бабл/переключение `stale`); отдельных правок в Phase 8 не потребовалось
- [x] T077 Проверить, что ни `dayText`, ни текст реплик не попадают в логи: пройтись по `app/lib/data/repositories/ai_reaction_repository_impl.dart`, `app/lib/core/network/ai_proxy_client.dart` и `AppLogger`-вызовам экрана (FR-034b) — подтверждено чтением кода: `_logFailure(code, characterId)` — единственный лог-вызов на этом пути, только код отказа и id персонажа, ни `dayText`, ни `reply`, ни сырой `DioException`/`error.message` (который у Dio может содержать тело запроса) не передаются; `ai_proxy_client.dart` не логирует вообще (никакого `LogInterceptor` в `Dio`); `logger` глушится целиком в release (`Level.off` при `kReleaseMode`) — двойная защита
- [x] T078 Сверить список включённых по умолчанию персонажей между `app/assets/characters/characters.json`, `SettingsRepositoryImpl` и `ReadOnlySettingsRepository` (FR-010b) — все три источника согласованы: `["cat","dog","crocodile","hippo"]` (drift-таблица `UserSettingsTable.enabledCharacterIds` `withDefault`, `read_only_repositories.dart`, `characters.json`); правок не потребовалось
- [x] T079 Проверить поведение при отсутствующем/битом каталоге персонажей — временно испортив `app/assets/characters/characters.json`: стол пуст, шкала настроения работает (FR-010d) — уже покрыто существующим тестом `table_cubit_test.dart`: `'a broken character catalog degrades to an empty table, not a screen error (FR-010d)'` (`CharacterCatalog.load()` возвращает `SerializationFailure`, `TableCubit` откатывается на `_catalog = const []` через `valueOrGet`, состояние остаётся `TableLoaded` с пустым `characters`, не `TableError`); на уровне виджета `data.characters.isEmpty` уже рисует `SizedBox.shrink()` вместо стола, оставляя шкалу/текст рабочими (`table_page.dart`) — реального повреждения ассета не делал, логика прослежена по коду и существующему тесту
- [~] T080 Замерить на устройстве базового класса (Android 10+, 4 ГБ ОЗУ) на release-сборке по процедуре `quickstart.md` §7: средний темп ≥55 FPS и ни одного кадра дольше 100 мс за 60 секунд, индикация ожидания появляется ≤2 с после тапа, бабл наполняется ≤15 с (SC-006, SC-007, FR-033) — **не выполнено, не может быть выполнено в этой среде**: FR-033 прямо запрещает измерение на симуляторе/эмуляторе («Симулятор и эмулятор для этой проверки не годятся»), а физического Android-устройства в этой песочнице нет. Требует отдельной сессии на реальном железе перед релизом
- [x] T081 Проверить, что release-сборка с пустым `PROXY_BASE_URL` падает с внятной ошибкой, а не уходит на заглушку (SC-012) — собран и установлен настоящий `--release` APK на Android-эмулятор (`flutter run -d emulator-5554 --release`, без `--dart-define=PROXY_BASE_URL`), сборка и установка прошли; визуально подтвердить экран `StartupErrorPage` не удалось — `adb screencap`/`uiautomator dump` зависали в этой песочнице (вероятно, ограничение рендер-бэкенда эмулятора, не связано с приложением), а `logger.e(...)` в `main.dart` печатает в лог только когда `kReleaseMode == false`, то есть в release-сборке намеренно молчит. Вывод сделан статическим разбором `main.dart`/`ai_proxy_config.dart`: `kReleaseMode` — константа времени компиляции, гарантированно `true` в `--release`; `AiProxyConfig.baseUrl` — `String.fromEnvironment('PROXY_BASE_URL')`, гарантированно `''` без `--dart-define`; `assertConfiguredForRelease()` при этой комбинации безусловно бросает `StateError`, перехватывается в `_start()` и заменяется на `runApp(StartupErrorPage(...))` — не на молчаливый переход к `StubAiProxyClient` (тот путь недостижим: до него код не доходит, `return` в `catch`-блоке). Логической развилки, которая могла бы дать другой исход, в коде нет
- [x] T082 Финальный gate: `flutter analyze` без ошибок и `flutter test` полностью зелёный; убедиться, что покрытие нового кода состояния не ниже 70% (SC-011) — 0 ошибок analyze, 229/229 тестов зелёных (227 + 2 новых `table_accessibility_test.dart`); `flutter test --coverage` по ключевым файлам экрана: `table_cubit.dart` 99.4%, `table_state.dart` 100%, `table_page.dart` 96.3%, `character_avatar.dart` 93.8%, `mood_scale_row.dart` 100%, `round_table_layout.dart` 95.3%, `day_text_field.dart` 87.5%, `speaking_bubble.dart` 74.2%, `character_catalog.dart` 90.5%, `ai_reaction_repository_impl.dart` 86.4% — все ≥70%; `ai_proxy_client.dart`/`share_service.dart` близки к 0% намеренно (реальный HTTP-клиент и `share_plus`-обёртка проверяются только против настоящего прокси/платформенного канала, не юнит-тестами — research.md R2/R14, R3)
- [~] T083 Полный ручной прогон `quickstart.md` §2–§7 — выполнено частично и не вживую: §1 (gate) выполнен реально; §2/§3 (US1–US5 на заглушке, пять отказов) покрыты автотестами `table_cubit_test.dart`/`table_page_test.dart`, включая пункт «Поделиться» (T069) — живого прогона на устройстве с открытием системного шаринг-листа не было (см. предыдущую сессию/T073); §5/§6 (офлайн-ядро, read-only) покрыты `table_cubit_test.dart`/`table_page_test.dart`; §7 (доступность) — `table_accessibility_test.dart` зелёный, ручная проверка на среднем устройстве не выполнена (см. T080); §4 (реальный прокси) недостижим — прокси-бэкенд ещё не существует (`project/prd/03-ai-integration.md`/`backend-proxy.md`, отдельная фаза)
- [x] T084 Записать в `project/process/lessons-learned.md` реальные грабли этой фазы (что сломалось → что делать вместо → почему это не разовая случайность); если ничего не ломалось — ничего не добавлять — добавлены два новых пункта: (1) `CustomSemanticsAction`/`OrdinalSortKey` не видны через `material.dart`/`widgets.dart` — нужен отдельный `import 'package:flutter/semantics.dart'`; (2) widget-тест `longPress`/`tap` промахивается на дефолтном 800×600 вьюпорте, когда контент (открывающийся вверх бабл) выходит за его границы без клипа — нужен точечный `tester.view.physicalSize`
...
- [x] T085 Freeze `CharacterAvatar`'s idle/talk Lottie animations (not just `SpeakingBubble`'s reveal effect) when `MediaQuery.disableAnimationsOf(context)` is true — e.g. `Lottie.asset(..., animate: !MediaQuery.disableAnimationsOf(context))` in `app/lib/presentation/table/widgets/character_avatar.dart` — per FR-033a (partial) — added `animate: !MediaQuery.disableAnimationsOf(context)` to the `Lottie.asset(...)` call in `_visual()`; new `app/test/widget/character_avatar_test.dart` constructs a `Character` fixture with non-null `idleAnimation`/`talkAnimation` (both are `null` for all four shipped MVP characters, so this path was previously untestable through the real asset) and asserts `LottieBuilder.animate` is `true` normally and `false` under `disableAnimations: true` for both `idle` and `speaking` states — 0 analyze errors, 231/231 tests green (229 + 2 new)
